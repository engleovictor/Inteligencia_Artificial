...
class graphAast(graph):
    def __init__(self):
        super().__init__()
        self.heurist = {}

    def mostrarMelhorCaminho(self, vert1: str, vert2: str, heurist_txt=""):
        ...
        while nv != vert2:
            ...
            for e in self.verts[nv]:
                if e.vert1 == nv:
                    heapq.heappush(ways, way(e.vert2,av,self.heurist[e.vert2] + pc + e.weight - self.heurist[nv]))
                else:
                    heapq.heappush(ways, way(e.vert1,av,self.heurist[e.vert1] + pc + e.weight - self.heurist[nv]))
